...
final allSalesByMonthOfYearProvider = FutureProvider.family
    .autoDispose<List<Sale>, DateTime>((ref, DateTime time) async {
  final allSales = ref.watch(allSalesProvider);
  List<Sale> sales = [];
  allSales.whenData((value) => sales = value);
  List<Sale> filteredSale = sales
      .where((element) =>
          element.soldOn.year == time.year &&
          element.soldOn.month == time.month)
      .toList();
  return filteredSale;
});
